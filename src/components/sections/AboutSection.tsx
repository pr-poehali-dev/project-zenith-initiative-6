...
const featureKeys = [
  ...
  { icon: "Crosshair", key: "pvp" },
  { icon: "Package", key: "trading" },
] as const
  ...
  return (
    <section id="about" className="px-6 py-24">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <p className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-4">{t.about.subtitle}</p>
          <h2 className="font-display text-3xl md:text-4xl font-bold text-zinc-100 mb-4">
            {t.about.title}
          </h2>
          <p className="text-zinc-500 max-w-xl mx-auto">{t.about.desc}</p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {featureKeys.map((f, i) => {
            const feature = t.about.features[f.key]
            return (
              <motion.div
                key={f.key}
                initial={{ opacity: 0, y: 20 }}
